...
class Header extends React.Component {
    ...
    renderLinks() {
        if(this.props.authenticated) {
            return [
            <li key={6}><Link to="/add">Add Interest</Link></li>,
            <li key={4}><Link to="/settings">Settings</Link></li>,
            <li id="signout" key={3} onClick={this.handleClick.bind(this)}><a>Sign Out</a></li>
            ]
            } else {
            return [
        ...
        return (
            <nav className="navbar navbar-inverse">
                <div className="container-fluid">
                    ...
                    <Link to="/" className="navbar-brand">Interest</Link >
                    </div>
